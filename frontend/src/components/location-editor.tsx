...
export function LocationEditor({ value, onChange, suggestions, costCenters, disabled = false }: { value: WorkLocation[]; onChange: (v: WorkLocation[]) => void; suggestions: string[]; costCenters: CostCenter[]; disabled?: boolean }) {
  ...
  return <div className="rounded-xl border bg-[#fbfcfc] p-3">
    <div className="mb-1 flex items-center justify-between"><Label className="flex items-center gap-2"><MapPin className="size-4 text-primary" /> Site, time and cost code</Label><span className="text-[11px] font-semibold text-amber-700">Cost code required · blank times = 8h day</span></div>
    ...
    {rows.map((row, index) => <div className="location-row" key={index}>
      <label className="field-label">Site<Input list="location-options" value={row.name} disabled={disabled} placeholder="e.g. 444 Pocatello" onChange={e => update(index, { name: e.target.value })} /></label>
      ...
      <label className="field-label">Site hours<Input type="number" min="0" max="24" step=".5" value={row.hours ?? ""} disabled={disabled} placeholder="Day defaults to 8h" onChange={e=>updateHours(index,e.target.value)} /></label>
      <div className="field-label"><span>Cost codes</span><div className="flex min-h-9 flex-wrap items-center gap-1 rounded-lg border bg-white px-2 py-1.5">{row.cost_centers.map(c => <span key={c.id} className="flex items-center gap-1 rounded-md bg-accent px-2 py-1 text-[11px] font-semibold text-primary">{c.name} · {c.id}<button disabled={disabled} onClick={() => update(index, { cost_centers: row.cost_centers.filter(x => x.id !== c.id) })}><X className="size-3" /></button></span>)}<input disabled={disabled} list="center-options" className="min-w-28 flex-1 border-0 bg-transparent text-xs outline-none" placeholder="Search or select" onChange={e=>{if(findCenter(e.currentTarget.value,true)){addCenter(index,e.currentTarget.value);e.currentTarget.value=""}}} onBlur={e=>{addCenter(index,e.currentTarget.value);e.currentTarget.value=""}} onKeyDown={e => { if (e.key === "Enter") { e.preventDefault(); addCenter(index, e.currentTarget.value); e.currentTarget.value = "" } }} /></div><small className="text-[10px] text-muted-foreground">Added codes appear as blue chips above.</small></div>
    ...
    <Button variant="ghost" size="sm" disabled={disabled} className="mt-3" onClick={addLocation}><Plus className="size-4" /> Add site</Button>
  </div>
}
