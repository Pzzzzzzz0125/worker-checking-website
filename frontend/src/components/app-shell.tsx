...
export function AppShell({ view, setView, reviewCount, children }: { view: View; setView: (v: View) => void; reviewCount: number; children: React.ReactNode }) {
  ...
  return <div className="app-grid">
    <aside className={cn("sidebar", open && "!flex !fixed inset-y-0 left-0 w-[270px]")}>
      ...
      <a href="http://localhost:7001" className="mt-4 flex items-center gap-3 rounded-xl bg-white/7 p-3 text-xs text-[#c8ddf8] hover:bg-white/10"><Smartphone className="size-5" /><span><strong className="block text-white">Foreman logger</strong>Mobile daily logging</span></a>
      <p className="mt-4 px-2 text-[10px] text-[#7fa6d8]">Developed by Zihao (Paul) Zhao</p>
    </aside>
    <main className="main">
      <header className="topbar"><div className="flex items-center gap-3"><Button className="md:hidden" variant="ghost" size="icon" onClick={() => setOpen(true)}><Menu className="size-5" /></Button><active.icon className="size-5 text-primary" /><span className="font-semibold">{active.label}</span></div><div className="hidden items-center gap-2 text-xs text-muted-foreground sm:flex"><span className="size-2 rounded-full bg-sky-500" />Shared database connected</div></header>
      {children}
    </main>
