...
function HoursTrend({data}:{data:Summary}){
  const points=data.trend||[]
  const maximum=Math.max(0,...points.map(point=>point.weighted_hours))
  const labelStep=Math.max(1,Math.ceil(points.length/8))
  const grouping=data.trend_resolution==="day"?"Daily":data.trend_resolution==="week"?"Weekly":"Monthly"
  return <Card className="mb-5"><CardHeader className="!flex-col justify-between gap-3 sm:!flex-row sm:items-start"><div><CardTitle>Hours trend</CardTitle><CardDescription>{grouping} regular hours with the additional overtime and double-time payroll weighting.</CardDescription></div><div className="flex flex-wrap gap-4 text-xs font-semibold text-muted-foreground"><span className="inline-flex items-center gap-1.5"><i className="size-2.5 rounded-sm bg-blue-600"/>Regular hours</span><span className="inline-flex items-center gap-1.5"><i className="size-2.5 rounded-sm bg-amber-500"/>OT/DT payroll premium</span></div></CardHeader><CardContent>
    {maximum>0?<div className="overflow-x-auto pb-1" role="img" aria-label={`${grouping} regular and weighted payroll hours chart`}><div className="flex h-60 min-w-[640px] items-end gap-1 border-b border-slate-200 px-1" style={{width:`max(100%, ${points.length*38}px)`}}>{points.map((point,index)=>{const regularPercent=point.regular_hours/maximum*100;const premium=Math.max(point.weighted_hours-point.regular_hours,0);const premiumPercent=premium/maximum*100;return <div className="flex h-full min-w-0 flex-1 flex-col justify-end" key={point.start}><div className="relative mx-auto w-[70%] max-w-9 flex-1" title={`${point.label}: ${compactNumber(point.regular_hours)} regular hours, ${compactNumber(point.weighted_hours)} weighted payroll hours`}><div className="absolute inset-x-0 rounded-t bg-amber-500" style={{height:`${premiumPercent}%`,bottom:`${regularPercent}%`}}/><div className="absolute inset-x-0 bottom-0 rounded-t bg-blue-600" style={{height:`${regularPercent}%`}}/></div><div className="h-9 pt-2 text-center text-[10px] font-medium text-muted-foreground">{(index%labelStep===0||index===points.length-1)?point.label:""}</div></div>})}</div></div>:<div className="grid h-44 place-items-center text-sm text-muted-foreground">No worked hours in this period.</div>}
  </CardContent></Card>
}
...
function DetailTable({title,rows,totalEstimatedCost}:{title:string;rows:any[];totalEstimatedCost?:number}){
  ...
  return <div><h3 className="mb-2 text-sm font-bold">{title}</h3><div className="table-wrap"><table className="data-table"><thead><tr><th>Name</th>{showCost?<><th>Regular hours</th><th>Weighted payroll hours</th><th>Estimated cost</th></>:<th>Hours</th>}</tr></thead><tbody>{rows.map((r:any)=><tr key={`${r.id||""}${r.name}`}><td><strong>{r.name}</strong>{r.id&&<small className="block text-muted-foreground">{r.id}</small>}</td>{showCost?<><td>{compactNumber(r.regular_hours)}h</td><td className="font-semibold text-primary">{compactNumber(r.weighted_hours)}h</td><td className="font-semibold tabular-nums">${compactNumber(r.estimated_cost)}</td></>:<td>{compactNumber(r.hours)}</td>}</tr>)}</tbody>{showCost&&totalEstimatedCost!==undefined&&<tfoot><tr className="border-t-2 bg-slate-100"><td colSpan={3}><strong>Total estimated cost</strong></td><td className="font-bold tabular-nums text-primary">${compactNumber(totalEstimatedCost)}</td></tr></tfoot>}</table>{!rows.length&&<div className="p-8 text-center text-xs text-muted-foreground">No details recorded.</div>}</div></div>
}
