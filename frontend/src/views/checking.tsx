...
function DetailTable({title,rows}:{title:string;rows:any[]}){
  const showCost=rows.some((row:any)=>row.estimated_cost!==undefined)
  return <div><h3 className="mb-2 text-sm font-bold">{title}</h3><div className="table-wrap"><table className="data-table"><thead><tr><th>Name</th><th>Hours</th>{showCost&&<th>Estimated cost</th>}<th>Days</th></tr></thead><tbody>{rows.map((r:any)=><tr key={`${r.id||""}${r.name}`}><td><strong>{r.name}</strong>{r.id&&<small className="block text-muted-foreground">{r.id}</small>}</td><td>{compactNumber(r.hours)}</td>{showCost&&<td className="font-semibold tabular-nums">${compactNumber(r.estimated_cost)}</td>}<td>{r.days}</td></tr>)}</tbody></table>{!rows.length&&<div className="p-8 text-center text-xs text-muted-foreground">No details recorded.</div>}</div></div>
}
