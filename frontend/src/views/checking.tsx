...
import { CalendarRange, Check, ChevronRight, Clock3, DollarSign, LoaderCircle, LockKeyhole, MapPin, RefreshCw, Search, Users } from "lucide-react"
...
type PayrollSort = "name_asc" | "name_desc" | "cost_desc" | "cost_asc" | "hours_desc" | "hours_asc" | "regular_desc" | "regular_asc"
type SiteSort = "name_asc" | "name_desc" | "cost_desc" | "cost_asc" | "hours_desc" | "hours_asc" | "days_desc" | "days_asc"
...
export function PayrollView({ bootstrap }: { bootstrap: Bootstrap }) {
  ...
  const workers=useMemo(()=>{
    ...
    const comparators:Record<PayrollSort,(a:any,b:any)=>number>={
      ...
      hours_desc:(a,b)=>b.hours-a.hours||name(a,b),
      hours_asc:(a,b)=>a.hours-b.hours||name(a,b),
  ...
  const show=async(id:number)=>{try{setDetail(await api(`/api/payroll_worker_detail?${new URLSearchParams({worker_id:String(id),from:data.period.from,to:data.period.to})}`))}catch(e){toast.error(e instanceof Error?e.message:String(e))}}
  ...
  return <div className="page">
    ...
    {data&&<>
      <div className="metric-grid mb-5">
        ...
        <Metric icon={Clock3} label="Hours" value={compactNumber(data.totals.hours)}/>
      ...
      <Card>
        <CardHeader className="!flex-col justify-between gap-3 sm:!flex-row sm:items-end">
          <div><CardTitle>{data.location}</CardTitle><CardDescription>{displayDate(data.range.from,true)} – {displayDate(data.range.to,true)} · Click a worker for site-specific history. W-2 estimates include California overtime weighting; 1099 estimates use straight time.</CardDescription></div>
          <label className="field-label min-w-56">Sort workers<select className="h-11 rounded-lg border bg-white px-3 text-sm" value={sort} onChange={e=>setSort(e.target.value as SiteSort)}><option value="name_asc">Name A–Z</option><option value="name_desc">Name Z–A</option><option value="cost_desc">Estimated cost: high to low</option><option value="cost_asc">Estimated cost: low to high</option><option value="hours_desc">Hours: high to low</option><option value="hours_asc">Hours: low to high</option><option value="days_desc">Days: high to low</option><option value="days_asc">Days: low to high</option></select></label>
        </CardHeader>
        <CardContent><div className="table-wrap"><table className="data-table">
          <thead><tr><th>Worker</th><th>Type</th><th>Hours</th><th>Estimated cost</th><th>Days</th><th>First day</th><th>Last day</th><th></th></tr></thead>
          <tbody>{workers.map((w:any)=><Fragment key={w.worker_id}>
            <tr className="cursor-pointer" onClick={()=>void show(w.worker_id)}>
              ...
              <td><strong className="text-primary">{compactNumber(w.hours)}h</strong></td>
            ...
            {detail?.worker.id===w.worker_id&&<tr className="bg-slate-50/70"><td colSpan={8} className="!p-4 sm:!p-6"><SiteWorkerDetail detail={detail}/></td></tr>}
          </Fragment>)}</tbody>
...
function SiteWorkerDetail({detail}:{detail:any}){
  return <div>
    <div className="mb-5 flex flex-wrap items-center justify-between gap-3">
      <div><h3 className="text-lg font-bold">{detail.worker.name} · {detail.location}</h3><p className="text-sm text-muted-foreground">{compactNumber(detail.totals.hours)} site hours · {compactNumber(detail.totals.weighted_hours)} weighted hours · estimated ${compactNumber(detail.totals.estimated_cost)}</p></div>
      <Badge variant={detail.worker.worker_type==="W2"?"warning":"secondary"}>{detail.worker.worker_type}</Badge>
    </div>
    <div className="table-wrap"><table className="data-table">
      <thead><tr><th>Date</th><th>Site hours</th><th>Cost codes</th><th>Regular</th><th>OT ×1.5</th><th>DT ×2</th><th>Weighted</th><th>Estimated cost</th></tr></thead>
