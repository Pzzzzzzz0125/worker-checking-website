...
export function WorkerEntryView({ bootstrap }: { bootstrap: Bootstrap }) {
 ...
 const openCopy=()=>{const dates=[...selected].sort();const first=dates[0]||month+"-01";setCopyMode("same");setTargetStart(first);setTargetEnd(first);setTargets(data?.worker?.id?[data.worker.id]:[]);setTargetSearch("");setCopyOpen(true)}
 ...
 const copyTargets=bootstrap.workers.filter(w=>w.name.toLowerCase().includes(targetSearch.trim().toLowerCase()));const shownTargets=copyTargets;const allShownSelected=shownTargets.length>0&&shownTargets.every(w=>targets.includes(w.id));const toggleShownTargets=()=>setTargets(current=>allShownSelected?current.filter(id=>!shownTargets.some(w=>w.id===id)):Array.from(new Set([...current,...shownTargets.map(w=>w.id)])))
