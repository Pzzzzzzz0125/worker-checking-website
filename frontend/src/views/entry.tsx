...
export function WorkerEntryView({ bootstrap }: { bootstrap: Bootstrap }) {
 ...
 const copy=async()=>{const rows=data.days.filter((d:Editable)=>selected.has(d.work_date));try{const result:any=await postJSON("/api/worker-days/copy",{source_worker_id:data.worker.id,target_worker_ids:targets,records:rows.map((r:Editable)=>({date:r.work_date,...payload(r)}))});toast.success(`Copied ${result.days} days to ${result.target_workers.length} workers.`);setCopyOpen(false);setTargetSearch("");setTargets([])}catch(e){toast.error(String(e))}}
