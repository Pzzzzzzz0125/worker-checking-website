...
export function DailyEntryView({ bootstrap }: { bootstrap: Bootstrap }) {
 ...
 const [records,setRecords]=useState<Editable[]>([])
 const [search,setSearch]=useState("")
 const [copied,setCopied]=useState<Editable|null>(null)
 const [selected,setSelected]=useState<Set<number>>(new Set())
 const [busy,setBusy]=useState(false)
 const draftKey=`speed-daily-draft:${date}`
 const persistDrafts=(rows:Editable[])=>{try{const drafts=rows.filter(row=>row.dirty);drafts.length?localStorage.setItem(draftKey,JSON.stringify(drafts)):localStorage.removeItem(draftKey)}catch{}}
 ...
 const load=async()=>{try{const data=await api<any>(`/api/day?date=${date}`);let rows=data.workers.map((x:any)=>normalize(x,date));try{const drafts=JSON.parse(localStorage.getItem(draftKey)||"[]");rows=rows.map((r:Editable)=>{const d=drafts.find((x:any)=>x.worker_id===r.worker_id);return d?{...r,...d,dirty:true}:r})}catch{}setRecords(rows);setSelected(new Set())}catch(e){toast.error(String(e))}}
 useEffect(()=>{void load()},[date])
 const setRow=(id:number,patch:Partial<Editable>)=>setRecords(old=>{const next=old.map(r=>r.worker_id===id?{...r,...patch,dirty:true}:r);persistDrafts(next);return next})
 const toggleExpanded=(id:number)=>setRecords(old=>old.map(row=>row.worker_id===id?{...row,expanded:!row.expanded}:row))
 const save=async(rows:Editable[],label="records")=>{
   if(!rows.length)return toast.info(`No ${label} to save.`)
   for(const r of rows){const error=validate(r);if(error){toast.error(`${r.worker_name}: ${error}`);return}}
   setBusy(true)
   try{await postJSON("/api/day",{date,records:rows.map(payload)});setRecords(old=>{const ids=new Set(rows.map(row=>row.worker_id));const next=old.map(r=>ids.has(r.worker_id)?{...r,dirty:false,existing:true}:r);persistDrafts(next);return next});toast.success(`Saved ${rows.length} ${rows.length===1?"worker":"workers"}.`)}catch(e){toast.error(String(e))}finally{setBusy(false)}
 }
 const clearRows=async(rows:Editable[],label:string)=>{
   const targets=rows.filter(row=>row.existing||row.dirty)
   if(!targets.length)return toast.info(`No ${label} to clear.`)
   if(!window.confirm(`Clear ${targets.length} ${targets.length===1?"record":"records"} for ${displayDate(date,true)}? Saved records will be deleted and unsaved drafts will be discarded.`))return
   setBusy(true)
   try{
     const saved=targets.filter(row=>row.existing)
     if(saved.length)await postJSON("/api/day/clear",{records:saved.map(row=>({worker_id:row.worker_id,date}))})
     const ids=new Set(targets.map(row=>row.worker_id))
     setRecords(old=>{const next=old.map(row=>ids.has(row.worker_id)?clearedRecord(row):row);persistDrafts(next);return next})
     setSelected(current=>new Set([...current].filter(id=>!ids.has(id))))
     toast.success(`Cleared ${targets.length} ${targets.length===1?"record":"records"}.`)
   }catch(e){toast.error(String(e))}finally{setBusy(false)}
 }
 const visible=records.filter(r=>r.worker_name?.toLowerCase().includes(search.toLowerCase()))
 const dirty=records.filter(r=>r.dirty)
 const meaningful=records.filter(r=>r.existing||r.dirty)
 const selectedMeaningful=records.filter(r=>selected.has(r.worker_id)&&(r.existing||r.dirty))
 const allVisibleSelected=visible.length>0&&visible.every(row=>selected.has(row.worker_id))
 const toggleVisible=()=>setSelected(current=>{const next=new Set(current);if(allVisibleSelected)visible.forEach(row=>next.delete(row.worker_id));else visible.forEach(row=>next.add(row.worker_id));return next})
 ...
 <Card><CardHeader className="!flex-col justify-between sm:!flex-row sm:items-center"><div><CardTitle>Workers for {displayDate(date,true)}</CardTitle><CardDescription>Drafts are kept in this browser until saved. “All” includes saved and edited records, not untouched blank workers.</CardDescription></div><div className="relative w-full sm:max-w-xs"><Search className="absolute left-3 top-3 size-4 text-muted-foreground"/><Input className="pl-9" value={search} onChange={e=>setSearch(e.target.value)} placeholder="Find worker…"/></div></CardHeader><CardContent className="grid gap-3">
   <div className="flex flex-wrap items-center gap-2 rounded-xl border bg-slate-50 p-3"><Badge>{selected.size} selected</Badge><Badge variant={dirty.length?"warning":"secondary"}>{dirty.length} edited</Badge><Button size="sm" variant="outline" onClick={toggleVisible}><Check className="size-4"/>{allVisibleSelected?"Deselect shown":"Select shown"}</Button><Button size="sm" disabled={busy||!meaningful.length} onClick={()=>save(meaningful,"records")}><Save className="size-4"/>Save all</Button><Button size="sm" disabled={busy||!selectedMeaningful.length} onClick={()=>save(selectedMeaningful,"selected records")}><Save className="size-4"/>Save selected</Button><Button size="sm" disabled={busy||!dirty.length} onClick={()=>save(dirty,"edited records")}><Save className="size-4"/>Save edited</Button><Button size="sm" variant="outline" className="text-red-600" disabled={busy||!selectedMeaningful.length} onClick={()=>clearRows(selectedMeaningful,"selected records")}><Trash2 className="size-4"/>Clear selected</Button><Button size="sm" variant="outline" className="text-red-600" disabled={busy||!meaningful.length} onClick={()=>clearRows(meaningful,"records")}><Trash2 className="size-4"/>Clear all</Button></div>
   {visible.map(r=><Card className={r.dirty?"border-amber-300":"shadow-none"} key={r.worker_id}><div className="p-4"><div className="flex flex-wrap items-center gap-3"><input type="checkbox" className="size-4 accent-[#2563eb]" checked={selected.has(r.worker_id)} onChange={e=>setSelected(current=>{const next=new Set(current);e.target.checked?next.add(r.worker_id):next.delete(r.worker_id);return next})}/><span className="avatar">{initials(r.worker_name)}</span><strong className="mr-auto">{r.worker_name}</strong><div className="flex rounded-lg bg-muted p-1"><Button size="sm" variant={r.status==="worked"?"default":"ghost"} onClick={()=>setRow(r.worker_id,workedPatch(r))}>Worked</Button><Button size="sm" variant={r.status==="sick_leave"?"default":"ghost"} onClick={()=>setRow(r.worker_id,sickLeavePatch())}>Sick leave</Button><Button size="sm" variant={r.status==="off"?"default":"ghost"} onClick={()=>setRow(r.worker_id,offPatch())}>Off</Button></div><Button size="sm" variant="ghost" onClick={()=>{setCopied(structuredClone(r));toast.success(`Copied ${r.worker_name}`)}}><Copy className="size-4"/>Copy</Button><Button size="sm" variant="ghost" disabled={!copied} onClick={()=>copied&&setRow(r.worker_id,{...structuredClone(copied),worker_id:r.worker_id,worker_name:r.worker_name,dirty:true})}><Clipboard className="size-4"/>Paste</Button><Button size="sm" disabled={busy||!r.dirty} onClick={()=>save([r])}>{busy?<LoaderCircle className="size-4 animate-spin"/>:<Save className="size-4"/>}Save</Button><Button size="sm" variant="ghost" className="text-red-600 hover:text-red-700" disabled={busy||!(r.existing||r.dirty)} onClick={()=>void clearRows([r],`${r.worker_name}'s record`)}><Trash2 className="size-4"/>Clear record</Button><Button variant="ghost" size="icon" onClick={()=>toggleExpanded(r.worker_id)}><ChevronDown className={`size-4 transition ${r.expanded?"rotate-180":""}`}/></Button></div>{r.expanded&&<RecordEditor record={r} bootstrap={bootstrap} update={p=>setRow(r.worker_id,p)}/>}</div></Card>)}
 </CardContent></Card>
 {dirty.length>0&&<div className="fixed bottom-20 right-5 z-40 flex flex-wrap items-center gap-2 rounded-2xl border bg-white p-3 pl-5 shadow-xl md:bottom-6"><span className="text-sm"><strong>{dirty.length}</strong> unsaved · draft protected</span><Button disabled={busy} onClick={()=>save(dirty,"edited records")}>{busy?<LoaderCircle className="size-4 animate-spin"/>:<Save className="size-4"/>}{busy?"Saving…":"Save edited"}</Button></div>}</div>
}
...
export function WorkerEntryView({ bootstrap }: { bootstrap: Bootstrap }) {
 const today=localISO();const [workerName,setWorkerName]=useState("");const [month,setMonth]=useState(today.slice(0,7));const [period,setPeriod]=useState<WorkerPeriod>("month");const [data,setData]=useState<any>(null);const [selected,setSelected]=useState<Set<string>>(new Set());const [copyOpen,setCopyOpen]=useState(false);const [targets,setTargets]=useState<number[]>([]);const [targetSearch,setTargetSearch]=useState("");const [copyMode,setCopyMode]=useState<"same"|"range">("same");const [targetStart,setTargetStart]=useState("");const [targetEnd,setTargetEnd]=useState("");const [copyPreview,setCopyPreview]=useState<CopyPreview|null>(null);const [existingMode,setExistingMode]=useState<"skip"|"replace">("skip");const [replaceConfirmed,setReplaceConfirmed]=useState(false);const [copyBusy,setCopyBusy]=useState(false);const [copyNotice,setCopyNotice]=useState("");const [saving,setSaving]=useState(false);const worker=useMemo(()=>bootstrap.workers.find(w=>w.name.toLowerCase()===workerName.toLowerCase())||bootstrap.workers.find(w=>w.name.toLowerCase().includes(workerName.toLowerCase())),[workerName,bootstrap])
 ...
 const setDay=(date:string,patch:Partial<Editable>)=>setData((d:any)=>({...d,days:d.days.map((r:Editable)=>r.work_date===date?{...r,...patch,dirty:true}:r)}))
 const toggleExpanded=(date:string)=>setData((current:any)=>({...current,days:current.days.map((row:Editable)=>row.work_date===date?{...row,expanded:!row.expanded}:row)}))
 const save=async(rows:Editable[],label="records")=>{if(!rows.length)return toast.info(`No ${label} to save.`);for(const r of rows){const error=validate(r);if(error)return toast.error(`${displayDate(r.work_date)}: ${error}`)}setSaving(true);try{await postJSON("/api/worker-days",{worker_id:data.worker.id,records:rows.map(r=>({date:r.work_date,...payload(r)}))});setData((d:any)=>({...d,days:d.days.map((r:Editable)=>rows.some(x=>x.work_date===r.work_date)?{...r,dirty:false,existing:true}:r)}));toast.success(`Saved ${rows.length} ${rows.length===1?"day":"days"}.`)}catch(e){toast.error(String(e))}finally{setSaving(false)}}
 const clearRows=async(rows:Editable[],label:string)=>{const targets=rows.filter(row=>row.existing||row.dirty);if(!targets.length)return toast.info(`No ${label} to clear.`);if(!window.confirm(`Clear ${targets.length} ${targets.length===1?"record":"records"} for ${data.worker.name}? Saved records will be deleted and unsaved drafts will be discarded.`))return;setSaving(true);try{const saved=targets.filter(row=>row.existing);if(saved.length)await postJSON("/api/day/clear",{records:saved.map(row=>({worker_id:data.worker.id,date:row.work_date}))});const dates=new Set(targets.map(row=>row.work_date));setData((d:any)=>({...d,days:d.days.map((item:Editable)=>dates.has(item.work_date)?clearedRecord(item):item)}));setSelected(current=>new Set([...current].filter(date=>!dates.has(date))));toast.success(`Cleared ${targets.length} ${targets.length===1?"record":"records"}.`)}catch(e){toast.error(String(e))}finally{setSaving(false)}}
 const resetCopyPreview=()=>{setCopyPreview(null);setExistingMode("skip");setReplaceConfirmed(false)}
 const openCopy=()=>{const dates=[...selected].sort();const first=dates[0]||month+"-01";setCopyMode("same");setTargetStart(first);setTargetEnd(first);setTargets([]);setTargetSearch("");resetCopyPreview();setCopyOpen(true)}
 const changeCopyMode=(mode:"same"|"range")=>{setCopyMode(mode);setTargets(mode==="range"&&data?.worker?.id?[data.worker.id]:[]);resetCopyPreview()}
 ...
 const sameWorkerSameDate=!!data?.worker?.id&&targets.includes(data.worker.id)&&targetDates.some(date=>selected.has(date));
 const copyPayload=()=>({source_worker_id:data.worker.id,target_worker_ids:targets,target_dates:targetDates,records:sourceRows.map((r:Editable)=>({date:r.work_date,...payload(r)}))})
 const validateCopyMapping=()=>{if(!sourceRows.length){toast.error("Select at least one source day.");return false}if(!targets.length){toast.error("Select at least one target worker.");return false}if(!targetDates.length){toast.error("Choose at least one target date.");return false}if(sourceRows.length>1&&sourceRows.length!==targetDates.length){setCopyNotice(`${sourceRows.length} source days cannot be copied to ${targetDates.length} target days. Select exactly ${sourceRows.length} target dates, or select only one source day to repeat.`);return false}if(sameWorkerSameDate){toast.error("You cannot copy a Worker to the same Worker on the same date. Choose a different target date.");return false}return true}
 const previewCopy=async()=>{if(!validateCopyMapping())return;setCopyBusy(true);try{const result=await postJSON<CopyPreview>("/api/worker-days/copy",{...copyPayload(),preview_only:true});setCopyPreview(result);setExistingMode("skip");setReplaceConfirmed(false)}catch(e){toast.error(String(e))}finally{setCopyBusy(false)}}
 const copy=async()=>{if(!copyPreview||!validateCopyMapping())return;if(existingMode==="replace"&&copyPreview.existing_records>0&&!replaceConfirmed){toast.error("Confirm that existing records may be replaced.");return}setCopyBusy(true);try{const result:any=await postJSON("/api/worker-days/copy",{...copyPayload(),existing_mode:existingMode,confirm_replace:existingMode==="replace"&&replaceConfirmed});const skipped=result.skipped_records||0;toast.success(`Copied ${result.written_records} ${result.written_records===1?"record":"records"}${skipped?`; skipped ${skipped} existing`:""}.`);setCopyOpen(false);setTargetSearch("");setTargets([]);resetCopyPreview();await load()}catch(e){toast.error(String(e))}finally{setCopyBusy(false)}}
 const periodDays=(data?.days||[]).filter((r:Editable)=>inWorkerPeriod(r.work_date,period));const dirtyDays:Editable[]=(data?.days||[]).filter((r:Editable)=>r.dirty);const dirtyPeriod=periodDays.filter((row:Editable)=>row.dirty);const meaningfulPeriod=periodDays.filter((row:Editable)=>row.existing||row.dirty);const selectedMeaningful=periodDays.filter((row:Editable)=>selected.has(row.work_date)&&(row.existing||row.dirty))
 ...
 const copyTargets=bootstrap.workers.filter(w=>w.name.toLowerCase().includes(targetSearch.trim().toLowerCase()));const shownTargets=copyTargets;const selectableShownTargets=shownTargets.filter(w=>!(copyMode==="same"&&w.id===data?.worker?.id));const allShownSelected=selectableShownTargets.length>0&&selectableShownTargets.every(w=>targets.includes(w.id));const toggleShownTargets=()=>{setTargets(current=>allShownSelected?current.filter(id=>!selectableShownTargets.some(w=>w.id===id)):Array.from(new Set([...current,...selectableShownTargets.map(w=>w.id)])));resetCopyPreview()}
 ...
 <Dialog open={copyOpen} onOpenChange={open=>{setCopyOpen(open);if(!open){setTargetSearch("");setTargets([]);resetCopyPreview()}}}>
  <DialogContent className="max-h-[90vh] overflow-y-auto"><DialogHeader><DialogTitle>Copy {selected.size} selected days</DialogTitle><DialogDescription>Multiple source days require the same number of target dates. One source day may repeat. Existing target records are skipped unless replacement is explicitly confirmed.</DialogDescription></DialogHeader>
   <div className="grid gap-3 rounded-xl border bg-slate-50 p-3"><div className="flex flex-wrap gap-2"><Button size="sm" variant={copyMode==="same"?"default":"outline"} onClick={()=>changeCopyMode("same")}>Use same dates</Button><Button size="sm" variant={copyMode==="range"?"default":"outline"} onClick={()=>changeCopyMode("range")}>Copy to date range</Button></div>{copyMode==="range"?<div className="grid gap-3 sm:grid-cols-2"><label className="field-label">Target from<Input type="date" value={targetStart} onChange={e=>{setTargetStart(e.target.value);resetCopyPreview()}}/></label><label className="field-label">Target to<Input type="date" value={targetEnd} onChange={e=>{setTargetEnd(e.target.value);resetCopyPreview()}}/></label></div>:<p className="text-xs text-muted-foreground">Target dates: {targetDates.map(date=>displayDate(date,true)).join(", ")||"—"}</p>}<div className="flex flex-wrap gap-3 text-xs font-semibold"><span className="text-primary">{sourceRows.length} source {sourceRows.length===1?"day":"days"}</span><span className={sourceRows.length>1&&sourceRows.length!==targetDates.length?"text-red-600":"text-primary"}>{targetDates.length} target {targetDates.length===1?"date":"dates"}</span></div></div>
   <div className="relative"><Search className="absolute left-3 top-3.5 size-4 text-muted-foreground"/><Input autoFocus className="pl-9" value={targetSearch} onChange={e=>setTargetSearch(e.target.value)} placeholder="Find target worker…"/></div><div className="flex items-center justify-between text-xs text-muted-foreground"><span>{targets.length} workers selected</span><Button type="button" size="sm" variant="ghost" onClick={toggleShownTargets}>{allShownSelected?"Clear shown":"Select shown"}</Button></div><div className="max-h-60 overflow-auto rounded-xl border p-2">{shownTargets.map((w:Worker)=><label key={w.id} className={`flex cursor-pointer items-center gap-3 rounded-lg p-2 hover:bg-muted ${copyMode==="same"&&w.id===data?.worker?.id?"opacity-50":""}`}><input type="checkbox" disabled={copyMode==="same"&&w.id===data?.worker?.id} checked={targets.includes(w.id)} onChange={e=>{setTargets(t=>e.target.checked?Array.from(new Set([...t,w.id])):t.filter(id=>id!==w.id));resetCopyPreview()}}/><span className="avatar">{initials(w.name)}</span><span>{w.name}{w.id===data?.worker?.id&&<small className="ml-2 text-muted-foreground">current worker · choose a different date</small>}</span></label>)}{!shownTargets.length&&<p className="p-5 text-center text-sm text-muted-foreground">No worker matches this search.</p>}</div>
   {copyPreview&&<div className="grid gap-3 rounded-xl border border-blue-200 bg-blue-50/60 p-4"><div className="grid grid-cols-3 gap-2 text-center"><div className="rounded-lg bg-white p-3"><strong className="block text-xl">{copyPreview.target_records}</strong><span className="text-xs text-muted-foreground">Target records</span></div><div className="rounded-lg bg-white p-3"><strong className="block text-xl text-blue-700">{copyPreview.empty_records}</strong><span className="text-xs text-muted-foreground">Empty</span></div><div className="rounded-lg bg-white p-3"><strong className="block text-xl text-amber-700">{copyPreview.existing_records}</strong><span className="text-xs text-muted-foreground">Already contain entries</span></div></div><div className="grid gap-2 sm:grid-cols-2"><label className={`cursor-pointer rounded-lg border p-3 text-sm ${existingMode==="skip"?"border-blue-500 bg-blue-100":"bg-white"}`}><input className="mr-2" type="radio" name="existing-mode" checked={existingMode==="skip"} onChange={()=>{setExistingMode("skip");setReplaceConfirmed(false)}}/><strong>Skip existing</strong><span className="mt-1 block text-xs text-muted-foreground">Recommended. Only empty target dates are written.</span></label><label className={`cursor-pointer rounded-lg border p-3 text-sm ${existingMode==="replace"?"border-red-400 bg-red-50":"bg-white"}`}><input className="mr-2" type="radio" name="existing-mode" checked={existingMode==="replace"} onChange={()=>setExistingMode("replace")}/><strong>Replace existing</strong><span className="mt-1 block text-xs text-muted-foreground">Overwrite records that already contain entries.</span></label></div>{existingMode==="replace"&&copyPreview.existing_records>0&&<label className="flex items-start gap-2 rounded-lg border border-red-200 bg-white p-3 text-xs text-red-800"><input className="mt-0.5" type="checkbox" checked={replaceConfirmed} onChange={e=>setReplaceConfirmed(e.target.checked)}/><span>I understand that {copyPreview.existing_records} existing {copyPreview.existing_records===1?"record":"records"} will be replaced.</span></label>}</div>}
   <div className="flex justify-end gap-2"><Button variant="ghost" disabled={copyBusy} onClick={()=>{setCopyOpen(false);setTargetSearch("");setTargets([]);resetCopyPreview()}}>Cancel</Button>{copyPreview?<Button disabled={copyBusy||(existingMode==="replace"&&copyPreview.existing_records>0&&!replaceConfirmed)} onClick={copy}>{copyBusy?<LoaderCircle className="size-4 animate-spin"/>:<Copy className="size-4"/>}{copyBusy?"Copying…":existingMode==="skip"?"Copy empty records":"Replace and copy"}</Button>:<Button disabled={copyBusy||!targets.length||!targetDates.length||sameWorkerSameDate} onClick={previewCopy}>{copyBusy?<LoaderCircle className="size-4 animate-spin"/>:<Search className="size-4"/>}{copyBusy?"Checking…":"Review copy"}</Button>}</div>
  </DialogContent>
 </Dialog>
 <Dialog open={Boolean(copyNotice)} onOpenChange={open=>!open&&setCopyNotice("")}><DialogContent><DialogHeader><DialogTitle className="flex items-center gap-2"><AlertTriangle className="size-5 text-amber-600"/>Date counts do not match</DialogTitle><DialogDescription>{copyNotice}</DialogDescription></DialogHeader><div className="flex justify-end"><Button onClick={()=>setCopyNotice("")}>Return to date selection</Button></div></DialogContent></Dialog>
 </div>
}
